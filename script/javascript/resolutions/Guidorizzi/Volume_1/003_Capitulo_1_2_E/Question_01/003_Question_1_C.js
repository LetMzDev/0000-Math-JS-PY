function initThreeJS ( containerId )
{
	...
	class NumberSystem
	{
		...
		createMarkers()
		{
			...
			const marker = new THREE.Mesh ( markerGeometry, markerMaterial );
			marker.position.set ( -4/3, 0, 0 );
			marker.rotation.x = Math.PI / 2;
			numberLine.add ( marker );

			this.createFractionNumber ( -4/3, -4, 3 );
		}
	...
	class MathematicalSolution
	{
		...
		createSolutionLine()
		{
			...
			for ( let x = -7; x <= -4/3; x += 0.05 ) // A solução é x <= -4/3
			{
		...
		createCriticalPoint()
		{
			...
			this.point.position.set ( -4/3, 0, 0 ); // Ponto crítico em -4/3
